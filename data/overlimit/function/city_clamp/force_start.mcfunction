...
scoreboard players set #cc_dwell overlimit.const 24000
function overlimit:city_clamp/try_start
execute unless score #cc_active overlimit.const matches 1 run return run tellraw @s {"text":"[overlimit] 開始に失敗した（1200以内の未クリアシティを確認）","color":"red"}
