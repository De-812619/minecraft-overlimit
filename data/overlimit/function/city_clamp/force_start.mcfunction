...
execute unless predicate overlimit:in_end_city run return run tellraw @s {"text":"[overlimit] エンドシティの中で実行してください","color":"red"}
tag @s remove overlimit.cc_tried
function overlimit:city_clamp/try_enter
execute unless score #cc_active overlimit.const matches 1 run return run tellraw @s {"text":"[overlimit] 開始に失敗した","color":"red"}
