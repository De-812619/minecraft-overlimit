...
execute unless predicate overlimit:in_fortress unless predicate overlimit:in_bastion run return run tellraw @s {"text":"[overlimit] 要塞かバストリオンの中で実行してください","color":"red"}
tag @s remove overlimit.nr_tried
function overlimit:nether_raise/try_enter
execute unless score #nr_active overlimit.const matches 1 run return run tellraw @s {"text":"[overlimit] 開始に失敗した","color":"red"}
