...
# #nr_ghast_alive は check_clear → nr_alive で設定済み
execute if score #nr_ghast_alive overlimit.const matches 1.. run scoreboard players set #nr_ghast_n overlimit.const 1
execute if score #nr_ghast_alive overlimit.const matches 1.. run scoreboard players set #nr_ghast_cd overlimit.const -1
...
title @a[predicate=overlimit:in_nether] actionbar {"translate": "overlimit.msg.nr.rankup","color":"red"}
