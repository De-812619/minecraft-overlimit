...
execute if score #nr_kind_id overlimit.const matches 1 run tellraw @a[predicate=overlimit:in_nether] ["",{"text":"ネザーレイズが始まった。要塞が蜂起している。","color":"gold"},{"text":" 要塞（","color":"yellow"},{"nbt":"target.x","storage":"overlimit:nr"},{"text":", ","color":"yellow"},{"nbt":"target.y","storage":"overlimit:nr"},{"text":", ","color":"yellow"},{"nbt":"target.z","storage":"overlimit:nr"},{"text":"）へ向かえ。","color":"yellow"},{"text":" （ヒート ","color":"gold"},{"score":{"name":"#heat","objective":"overlimit.const"},"color":"gold"},{"text":"）","color":"gold"}]
