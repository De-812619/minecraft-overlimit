# #minecraft:load の実行位置はワールドスポーン。Marker で X/Y/Z を取る。
...
execute as @e[type=minecraft:marker,tag=overlimit.spawn_cap,limit=1] store result score #bw_spawn_y overlimit.const run data get entity @s Pos[1]
...
execute unless score #bw_spawn_y overlimit.const matches -2147483648.. run scoreboard players set #bw_spawn_y overlimit.const 64
