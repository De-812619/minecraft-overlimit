# ネザーと同様、約4秒溜めてから転送。歪みは吐き気（ポータル実ブロックは使わない）。
...
execute if score @s overlimit.portal_charge matches 1 run effect give @s minecraft:nausea 8 0 true
execute at @s anchored eyes run particle minecraft:portal ^ ^ ^0.4 0.15 0.15 0.15 0.4 6
execute if score @s overlimit.portal_charge matches 80.. run function overlimit:portal/try_warp
