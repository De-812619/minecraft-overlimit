...
3. **追加エンチャント（レア）**
  下表の各行を独立判定。当たった枠は `set_enchantments`（`add: false`）で**そのエンチャントだけを表のレベルに上書き**する。通常枠のレベルには加算しない（加算すると 11 以上になり、`enchantment.level.13` のような欠落と金床費用の膨張が起きる）。
...
精錬とハイパーディグ、暴食とソウルテイカー、**ハイパーディグと効率強化**、**クロスボウの無限と修繕**は排他（両方当たった場合は一方のみ）。
