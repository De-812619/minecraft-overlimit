...
- バストリオンの開始高さは Y=50。ネザー帯に加え浜辺にも出る。ピグリンはゾンビ化せず、WARNING / DANGER / CRISIS になる
...
最寄プレイヤーが発光し、ボスバー「ネザーオーバーフロー  WARNING  30分0秒」（ゲージは残り時間）がすぐ出る。門から 32 以上離れている間はピグリン／ブルート／ガストは出ない（タイマーは進む）。32 以内に入ると開始バースト8、以降 Ph1 は約2体/秒・同時12まで。近くのプレイヤーへ向かう（コア所持者を優先しない。金防具でも中立にならない）。ピグリン／ブルートはランク色で発光する（WARNING金／DANGER赤／CRISIS紫。ガストは光らない）。15秒経ってもゾンビ化しない。**ガストは同時最大1体・各フェーズ5回まで**（WARNING名は付かない。倒すと1〜60秒後に再出現。5回出すとそれ以上は出ない）。Peaceful では始まらない。一度も門に近づいていないと始まらない。
