...
```mcfunction
/loot give @s loot overlimit:golden_bow
```

または `/function overlimit:item/golden_bow/give`。セット用リソースパック有効時はオリジナル見た目（引き絞り3段階）、名前は「黄金弓」、耐久 768。矢が必要（無限はエンチャントした場合のみ）。フル引きの矢は通常の弓より速く、ダメージも約2倍。命中した相手が発光すること。金床で金ブロックを並べて修理できること。エンチャント台に載せて弓と同じエンチャントが付くこと。

クラフト（弓を中心、周囲8マスに金ブロック）:
...
## 4. 強化Mob（WARNING / DANGER / CRISIS / DISASTER）
...
何度か召喚し、名前（金 `WARNING` / 赤 `DANGER` / 紫 `CRISIS` / 青 `DISASTER`）・体力・ヘルメットを見る。XP は倒してオーブ量を通常個体と比較する。
...
/execute as @e[type=minecraft:zombie,limit=1,sort=nearest] run function overlimit:mob/make_disaster
```

CRISIS は体が 1.5 倍、DISASTER は 1.8 倍になる（頭上 4 マスが塞がっていると DISASTER は拡大しない）。既に名前があるMobや、スキャン済み（一度スポーンして判定済み）の個体は自然ロールの対象外。

XP の確認（見た目では分かりにくい。`/kill` では経験値が落ちないので、殴って倒す）:

1. ゾンビを召喚して `make_danger`（または warning / crisis / disaster）
2. サバイバルで倒す（すぐ拾わない）
3. オーブの Value を見る
...
ゾンビの合計は通常 5。WARNING なら約 7、DANGER なら 50、CRISIS なら 100、DISASTER なら 200。DANGER 以上はオーブが大きくオレンジ核になる。
...
| エンチャント | 確認の目安 |
| :--- | :--- |
| 超重力 | 近接で敵を殴る → ヒット位置に領域（3秒）。半径3の敵がその場で拘束（Slowness）。引き寄せなし。1プレイヤー=1領域（他プレイヤーのは消さない）。CD6秒（portal＋完了音）。プレイヤー・ダメージなし |
| ネクロマンシー | **タイトルへ戻って入り直し**後、ネクロ剣で名前なし敵を倒す → ヒット音のあと、倒した種の友好Mob（魂・通常の半分サイズ）が出現。WARNING／DANGER／CRISIS／DISASTER／ボス除外 |
...
地上ではほぼ必ず WARNING / DANGER / CRISIS / DISASTER になり、感知が広い。スポナーの隣・試練の間では 100% にならない（通常の 16.8% はあり）。

開始直後に自分から **12〜16ブロックの地表（自分の高さ±4）** へ WARNING 等の強化敵が約10体出る。バニラの敵自然スポーンは止まる。以降は **約4秒ごとに最大2体**。その直前に、自分から **25ブロック以上**（または大きく高さが違う）イベント敵は消える（撃破点は増えない）。**松明などの光源ブロックから水平16ブロック以内には追加しない**。顔の前（6未満）や谷底・地下にも追加しない。撃破するとボスバーの `N/100体` が増える。**CRISISを倒すと+3、DISASTERを倒すと+5**。Peaceful では追加しない。終了（または `/function overlimit:blood_moon/force_end`）で `spawn_monsters` が戻る。
...
- 地形がオーバーワールドと同系統（洞窟あり）。砂漠・ジャングル・沼・悪地あたりはネザーバイオーム。浜辺は砂のまま（ネザー化しない）
- バストリオンの開始高さは Y=50。ネザー帯に加え浜辺にも出る。ピグリンはゾンビ化せず、WARNING / DANGER / CRISIS / DISASTER になる
- 本ネザーの要塞・バストリオン、オーバーワールドの浜辺は今までどおり（BW専用バイオームなので本世界には出ない）
...
| 手順 | 期待 |
| --- | --- |
| 地上の開けた場所で `/summon minecraft:zombie ~ ~ ~` を数回 | ほぼ全部 WARNING / DANGER / CRISIS / DISASTER |
| スポナーの隣（4マス以内）で同じ | 多くは名無し。16.8% だけ強化（100% なら失敗） |
...
仕様は [NETHER_OVERFLOW.md](./NETHER_OVERFLOW.md)。

デプロイ → `/reload` 後、**一度ネザーポータルに近づいた状態なら、オーバーワールドのどこからでも**:

```mcfunction
/function overlimit:nether_overflow/force_start
```

最寄プレイヤーが発光し、ボスバー「ネザーオーバーフロー  WARNING  30分0秒」（ゲージは残り時間）がすぐ出る。門から 32 以上離れている間はピグリン／ブルート／ガストは出ない（タイマーは進む）。32 以内に入ると開始バースト8、以降 Ph1 は約2体/秒・同時12まで。近くのプレイヤーへ向かう（コア所持者を優先しない。金防具でも中立にならない）。ピグリン／ブルートはランク色で発光する（WARNING金／DANGER赤／CRISIS紫／DISASTER青。ガストは光らない）。15秒経ってもゾンビ化しない。**ガストは同時最大1体・各フェーズ5回まで**（WARNING名は付かない。倒すと1〜60秒後に再出現。5回出すとそれ以上は出ない）。Peaceful では始まらない。一度も門に近づいていないと始まらない。
...
```mcfunction
/function overlimit:nether_overflow/force_fail
```

ボスバーが消え、イベント敵が消える。その直後から約1秒、門中心の地表がネザー化する。8マス四方ごとにバイオームがまとまる（ネザー荒地／真紅の森／歪んだ森／ブラックストーン／玄武岩デルタ）。真紅・歪んだナイリウムの区画には、骨粉で育てたのと同じ巨大キノコが疎らに立ち、足元に小さなキノコや根が散る。建築・水・既存の木は残る。同じ門の2回目は半径が 8→12→16→20 と広がる。別のネザーポータルで始めると初回8に戻る。

途中で止まっているとき（スポーンが続く・ボスバーが残る）:

```mcfunction
/function overlimit:nether_overflow/force_end
```

時間切れ（ネザー化）:

```mcfunction
/function overlimit:nether_overflow/force_timeout
```

ボスバーが消え、イベント敵が消え、門周辺がネザー化する。タイトルは「時間切れ」。

勝利の終了だけ見るとき（発生中に）:

```mcfunction
/function overlimit:nether_overflow/end_victory
```

報酬チェストが出て、敵・ボスバーが消える（ネザー化しない）。門の正面（歩行方向の2〜3マス）に出る。ネザー化済みのネザーラックや巨大キノコの上でも、そのマスをチェストに置き換える。
...
- ブラッドムーン強制開始中は本イベントが始まらない。本イベント中の夕方に BM が始まらない
- コア所持者が死亡、または制限時間切れで失敗（ネザー化）。被弾では敗北しない
- Ph3 のピグリン／ブルートを消化して場が空なら勝利（ガストは倒さなくてよい。残っていても勝つ）。Ph1/2 で空なら次フェーズへ繰り上げ。30分放置は勝利にならない
- ガストは同時1、フェーズごとに最大5。倒してから次が出るまで1〜60秒。フェーズが変わると回数はリセット（生き残った1体は次の5回に含む）
- 所持者がネザーに入るとコアが OW の別人へ移る。OW が空なら停止して再開
- 枠を壊してもイベントは門座標で続く
- 終了でイベント敵が消え、名札付きとネクロは残る
- ブラッドゲートだけでは始まらない
- 金防具でもピグリン／ブルートが殴ってくる。コア以外が近くにいればそちらも狙われる（コア所持者だけを追わない）
- 門から離れた場所で強制開始してもボスバーとタイマーは始まる。32以内に来るまで敵は出ない。来なければ時間切れでネザー化する
