...
100体クリアの報酬は、オーバーワールドにいる状態でイベント産の強化敵を倒してテキストが `100/100体` になるまで。ネザーにいると出ない。**本人の隣にチェスト**（「ブラッドムーンの報酬」）。死亡中でも死亡位置に出る。本の中身だけ見るときは上の `/loot give`。
...
- 100体で本、朝で本なし、残敵が消える。クリア時は隣に報酬チェスト
