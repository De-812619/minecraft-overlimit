...
デプロイ → `/reload` 後、**オーバーワールドのネザーポータルから 32 ブロック以内で**:

```mcfunction
/function overlimit:nether_overflow/force_start
```

最寄プレイヤーが発光し、ボスバー「ネザーオーバーフロー  WARNING  30分0秒」（ゲージは残り時間）、門の半径16にピグリン／ブルートがすぐ出る（開始バースト8、以降 Ph1 は約2体/秒・同時12まで）。コア所持者へ向かう。ピグリン／ブルートはランク色で発光する（WARNING金／DANGER赤／CRISIS紫。ガストは光らない）。15秒経ってもゾンビ化しない。**ガストは同時最大1体・各フェーズ5回まで**（WARNING名は付かない。倒すと1〜60秒後に再出現。5回出すとそれ以上は出ない）。Peaceful では始まらない。門に立たなくても、32 以内なら始まる。
